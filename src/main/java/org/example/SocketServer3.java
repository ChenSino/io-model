        ...
        try {
            ...
            while(true) {
                ...
                try {
                    ...
                    System.out.println("新连接进入，客户端端口："+socket.getPort());
                    ...
                    synchronized (SocketServer3.xWait) {
                        SocketServer3.LOGGER.info("这次没有从底层接收到任何TCP连接，等待1秒，模拟事件X的处理时间");
                        SocketServer3.xWait.wait(1000);
                    }
                    continue;
                }
                ...
                BIORead:while(true) {
                    try {
                        ...
                        //      主线程在这里又可以做一些事情，记为Y
                        //===========================================================
                        SocketServer3.LOGGER.info("这次没有从底层接收到任务数据报文，等待1秒，模拟事件Y的处理时间");
                    }
                }
                //下面打印信息
